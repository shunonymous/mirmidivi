/* message.hpp - Enums for MIDI messages
 *
 * Picked from fluidsynth(/src/midi/fluid_midi.h), modified for mirmidivi 
 *
...
namespace mirmidivi
{
    ...
    const short MAX_NUMBER_OF_TRACKS = 128;
...
/* MIDI SYSEX useful manufacturer values */
    ...
    const short MIDI_SYSEX_DEVICE_ID_ALL = 0x7F;    /**< Device ID used in SYSEX messages to indicate all devices */

/* SYSEX sub-ID #1 which follows device ID */
    const short MIDI_SYSEX_MIDI_TUNING_ID = 0x08;    /**< Sysex sub-ID #1 for MIDI tuning messages */
    const short MIDI_SYSEX_GM_ID          = 0x09;    /**< Sysex sub-ID #1 for General MIDI messages */
...
/* General MIDI sub-ID #2 */
    const short MIDI_SYSEX_GM_ON  = 0x01;    /**< Enable GM mode */
    const short MIDI_SYSEX_GM_OFF = 0x02;    /**< Disable GM mode */
